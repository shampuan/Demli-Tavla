...
class ScoreWindow(QMessageBox):
    def __init__(self, kazanan, puan, sure, parent=None):
        ...
        self.setText("Tebrikler, oyun tamamlandı!")
...
class Pul(QLabel):
    ...
    def mousePressEvent(self, event):
        # Önce oyunun duraklatılıp duraklatılmadığını veya bitip bitmediğini kontrol ediyoruz
        if not self.main_window.game_started: return # Oyun bittiyse hareket etme
...
class DemliTavla(QMainWindow):
    def __init__(self):
        ...
        self.settings_file = os.path.join(self.config_dir, "ayarlar.json")
        
        # Temel Değişkenler (init_ui'den önce tanımlanmalı)
        ...
        self.ses_ortam_acik = True
        self.ses_zar_acik = True
        self.load_settings()
        ...
        if not self.ses_ortam_acik:
            self.chan_ambient.pause()
        self.play_jingle("mirkelam_tavla.mp3", self.vol_mirkelam_giris)
        
        # İlk açılıştaki nokta gibi görünme sorununu çözmek için görünümü yeniliyoruz
    ...
    def init_ui(self):
        ...
        self.resize(800, 525)
        # Sanal oyun alanını (Scene) orijinal boyutlarımızda oluşturuyoruz
        self.game_scene = QGraphicsScene(0, 0, 800, 525)
        
        # Mevcut tüm arayüz bileşenlerini barındıracak olan kapsayıcı widget
        self.central_widget = QWidget()
        self.central_widget.setFixedSize(800, 525)
        self.main_layout = QHBoxLayout()
        self.central_widget.setLayout(self.main_layout)
        
        # Arayüz widget'ını sahneye ekliyoruz
        self.game_scene.addWidget(self.central_widget)
        
        # Sahneyi ekranda gösterecek ve ölçekleyecek olan ana View bileşeni
        self.main_view = QGraphicsView(self.game_scene, self)
        self.setCentralWidget(self.main_view)
        
        # Kaydırma çubuklarını gizliyoruz ve yumuşatma ayarlarını açıyoruz
        self.main_view.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.main_view.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        from PyQt5.QtGui import QPainter
        self.main_view.setRenderHint(QPainter.Antialiasing)
        self.main_view.setRenderHint(QPainter.SmoothPixmapTransform)
        ...
        self.top_row.addWidget(self.status_image_label)
        ...
        self.dice_layout.addStretch() # Zarları sola yaslar
       ...
       # Butonlar Kontrolü
        ...
        self.cay_label.setFixedSize(240, 240)  # Taşmayı önlemek için boyut sabitlendi
        self.cay_label.setAlignment(Qt.AlignCenter)
        self.cay_movie = QMovie(self.get_path("turkish_cay.gif"))
        ...
        self.side_panel.addWidget(self.cay_label) # Gifi en tepeye aldık
        self.side_panel.addWidget(self.btn_roll)
        self.side_panel.addWidget(self.btn_flip)
        # Geri al butonunu da side_panel'e eklediğinden emin ol (Eğer önceden ekliyse buraya taşıyabilirsin)
    ...
    def save_for_undo(self):
        """Durumu tam kopya olarak dondurur."""
    ...
    def undo_move(self):
        ...
        # 1. Pulların mantıksal hane bilgilerini geri yükle
        for pul, hane in last_state['pul_positions'].items():
            pul.current_hane = hane
            
        # 2. Tahtadaki pulları görsel olarak yerlerine taşı
        for h_id, pullar_listesi in self.board_state.items():
            for i, p in enumerate(pullar_listesi):
                ...
                # Çünkü x_points ve y_points listelerin sabit, ama hane dizilimin değişiyor.
                is_top = h_id <= 11
    ...
    def update_turn_visual(self):
        ...
        if not pix.isNull():
            # Sıranın kimde olduğunu pulla gösteriyoruz
    ...
    def update_moves_display(self):
        ...
        if not self.available_moves:
            ...
            # Hamle kalmadıysa zarları soluklaştır
            self.set_dice(self.d1, self.last_d1, soluk=True)
            self.set_dice(self.d2, self.last_d2, soluk=True)
            ...
            self.set_dice(self.d1, self.last_d1, soluk=False)
            self.set_dice(self.d2, self.last_d2, soluk=False)
    ...
    def create_menu(self):
        ...
        s = m.addMenu("Ses")
        self.action_ses_ortam = QAction("Kıraathane Müziği", self, checkable=True)
        self.action_ses_ortam.setChecked(self.ses_ortam_acik)
        self.action_ses_ortam.triggered.connect(self.toggle_ses_ortam)
        self.action_ses_zar = QAction("Zar Sesleri", self, checkable=True)
        self.action_ses_zar.setChecked(self.ses_zar_acik)
        self.action_ses_zar.triggered.connect(self.toggle_ses_zar)
        s.addAction(self.action_ses_ortam)
        s.addAction(self.action_ses_zar)
    ...
    def set_dice(self, label, val, soluk=False):
        pix = QPixmap(self.get_path(f"dice_{val}.png")).scaled(72, 72, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        if soluk:
            from PyQt5.QtGui import QPainter, QColor
            soluk_pix = QPixmap(pix.size())
            soluk_pix.fill(Qt.transparent)
            painter = QPainter(soluk_pix)
    ...
    def roll_dice_anim(self):
        self.undo_stack = [] # Yeni tur başladı, eski geçmişi temizle
    ...
    def anim_step(self):
        ...
        if self.r_cnt > 12:
            self.t.stop()
            if self.ses_zar_acik:
                self.play_effect("dices_stopped.mp3", self.vol_zar_durma)
            self.dice_rolled = True
            self.last_d1, self.last_d2 = v1, v2
    ...
    def end_turn(self):
        ...
        if not self.is_paused:
            ...
            # Eğer sıra Koyu'daysa (AI) ve oyun devam ediyorsa, AI'yı başlat
                ...
                QTimer.singleShot(500, self.run_ai_turn) # 1 saniye sonra AI zar atar
    ...
    def blink_roll_button(self):
        ...
        def toggle_style():
            if self.blink_count < 6:  # 3 kez yanıp sönme (aç-kapat toplam 6 adım)
    ...
    def pause_game(self):
        ...
        self.is_paused = True
        self.pause_start = time.time() # Duraklatma anını kaydet
    ...
    def resume_game(self):
        ...
        self.is_paused = False
        # Geçen süreyi ana süreden çıkararak oyun süresini doğru hesapla
        ...
        if os.path.exists(self.settings_file):
            try:
                with open(self.settings_file, "r", encoding="utf-8") as f:
                    ayarlar = json.load(f)
                self.ses_ortam_acik = ayarlar.get("ses_ortam_acik", True)
                self.ses_zar_acik = ayarlar.get("ses_zar_acik", True)
            except:
                pass

    def save_settings(self):
        ayarlar = {
            "ses_ortam_acik": self.ses_ortam_acik,
            "ses_zar_acik": self.ses_zar_acik
        }
        with open(self.settings_file, "w", encoding="utf-8") as f:
            json.dump(ayarlar, f, ensure_ascii=False, indent=4)

    def toggle_ses_ortam(self):
        self.ses_ortam_acik = self.action_ses_ortam.isChecked()
        if self.ses_ortam_acik:
            self.chan_ambient.unpause()
        else:
            self.chan_ambient.pause()
        self.save_settings()

    def toggle_ses_zar(self):
        self.ses_zar_acik = self.action_ses_zar.isChecked()
        self.save_settings()
    ...
    def update_pul_visual_pos(self, pul, hane_id, index):
        ...
        if self.is_flipped:
            # Senin board_area genişliğin 533, yüksekliğin 468. 
    ...
    def resizeEvent(self, event):
        """Pencere boyutu değiştikçe sahneyi en-boy oranını koruyarak ölçekler."""
        if event is not None:
            super().resizeEvent(event)
        if hasattr(self, 'main_view') and hasattr(self, 'game_scene'):
            from PyQt5.QtCore import QRectF
            self.main_view.fitInView(
                QRectF(0, 0, 800, 525), 
